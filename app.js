...
function submitDeposit() {
    ...
    const method = document.getElementById('dep-method').value; // Capture selected method
    if(!amount || !ref) return alert("Missing info");
    const key = db.ref('requests/deposits').push().key;
    const req = { id: key, user: currentUser, amount, ref, method: method, status: 'pending', type: 'Deposit' }; 
...
function showAdminPrompt() { 
    // This is the corrected part: The button to show admin should trigger the prompt directly
    const password = prompt("Enter Admin Password:");
    if (password === "Propetas12") {
        switchTab('admin'); 
        loadAdmin(); 
    } else if (password !== null) { // If user entered something and it's wrong
        ...
        ['deposits', 'withdrawals'].forEach(cat => {
            if(data[cat]) Object.values(data[cat]).forEach(req => {
                ...
                div.innerHTML += `<div class="bg-white p-3 rounded-xl border text-[10px]">
                    ...
                    <p>${req.ref || req.address} | Method: ${req.method || 'N/A'}</p> 
