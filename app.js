...
const MAX_ACCUMULATION = 4 * 60 * 60 * 1000; 
...
function updateUI() {
    ...
    // Dragon UI
    const dragonBox = document.getElementById('dragon-timer');
    if (userData.dragonEngineerUntil > Date.now()) {
        dragonBox.classList.remove('hidden');
        document.getElementById('dragon-time-left').innerText = formatMs(userData.dragonEngineerUntil - Date.now());
    } else { dragonBox.classList.add('hidden'); }
...
function claimFlower(index) {
    ...
    const plantKey = Object.keys(rawPlants)[index];
...
function buyDragonEngineer() {
    ...
    const until = Math.max(userData.dragonEngineerUntil || 0, Date.now()) + (24 * 60 * 60 * 1000);
    db.ref('users/' + currentUser).update({
        balance: userData.balance - cost,
        dragonEngineerUntil: until
    });
}
...
function autoHarvestLogic() {
    ...
    Object.keys(rawPlants).forEach(key => {
        ...
        if (plant.isFinished) return;
        const timeSinceLast = now - plant.lastClaim;
        if (timeSinceLast > 30000) { // Every 30s
            const rate = FLOWER_TYPES[plant.type].roi / FLOWER_TYPES[plant.type].duration;
...
function submitDeposit() {
    ...
    const req = { id: key, user: currentUser, amount, ref, method: 'Deposit', status: 'pending', type: 'Deposit' };
